...
# -*- coding: utf-8 -*-
"""
    notebook_reload_hotkeys.py
    ~~~~~~~~~~~~~~~~~~~~~~~~~~

    This File is ment for testing purposes to enhance the notebookwidget by Uri Wickert
    with a keyboard - hotkey handling.

    Problem description:
        Think about two Widgets, lets call them "Project settings" and "editor".
        On "Project settings" jou can create a new Project, on "editor" you can create a new file.
        Whats more obvious to call both actions with the Keyboard hotkey:

            CTRL + n            --> "n" stands here for new (surprise, surprise ;-) )

        The Problem is to bind two Actions to one keyboard - hotkey!

    General solving:
        On switching from "Project settings" to "editor" the Keyboard - Hotkeys must be rebind
        to the action provided by the actual widget.


    Idea:
        Every tab knows all availiable Keyboard - Hotkeys and its own.
        When a tab is displayed all Keyboard - Hotkeys will be unbound(or only those which are bind on two actions)

        to switch the tabs themselves by a prdefined Keyboard - Hotkey there could be a function under main() which binds a Hotkey to 'n.display(editor_frame)'
...
class Main_GUI:
    def __init__(self, parent_widget=None):
        ...
        parent_widget.bind("<F1>", self.switch_tab)
